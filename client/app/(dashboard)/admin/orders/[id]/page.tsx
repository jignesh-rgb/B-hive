...
const AdminSingleOrder = () => {
    ...
    const updateOrder = async () => {
        if (
            ...
            apiClient.put(`/api/orders/${order?._id}`, order)
                .then(() => {
    ...
    const deleteOrder = async () => {
        apiClient.delete(`/api/order-product/${order?._id}`)
            .then(() => {
                apiClient.delete(`/api/orders/${order?._id}`)
                    .then(() => {
            ...
            <div className="flex flex-col gap-y-7 xl:ml-5 w-full max-xl:px-5">
                ...
                <div className="mt-5">
                    <label className="w-full">
                        <div>
                            ...
                            <span className="text-base"> {order?._id}</span>
                        </div>
                    </label>
                </div>
                ...
                <div>
                    {orderProducts?.map((product) => (
                        <div className="flex items-center gap-x-4" key={product?._id}>
                            <Image
                                src={getImageSrc(product?.product?.mainImage)}
                                alt={
                                    product.product?.title
                                        ? `Product image of ${product.product.title}`
