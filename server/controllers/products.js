...
const createProduct = asyncHandler(async (request, response) => {
  ...
  if (Array.isArray(data)) {
    if (data.length === 0) {
      throw new AppError("Product array cannot be empty", 400);
    }

    // Validate each product
    for (const product of data) {
      if (!product.title) throw new AppError("Missing required field: title", 400);
      if (!product.slug) throw new AppError("Missing required field: slug", 400);
      if (!product.price) throw new AppError("Missing required field: price", 400);
      if (!product.merchantId) throw new AppError("Missing required field: merchantId", 400);
      if (!product.categoryId) throw new AppError("Missing required field: categoryId", 400);

      if (!mongoose.Types.ObjectId.isValid(product.merchantId)) {
        throw new AppError(`Invalid merchantId: ${product.merchantId}`, 400);
      }

      if (!mongoose.Types.ObjectId.isValid(product.categoryId)) {
        throw new AppError(`Invalid categoryId: ${product.categoryId}`, 400);
      }
    }
  ...
  if (!mongoose.Types.ObjectId.isValid(merchantId)) {
    throw new AppError("Invalid merchantId", 400);
  }
